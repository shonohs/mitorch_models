...
class SSDLiteExtraLayers3(Head):
    """Add three additional layers for SSDLite."""
    def __init__(self, backbone, width_multiplier=1):
        m = width_multiplier
        base_output_shapes = Head.get_base_output_shapes(backbone, [4, 5])
        out_channels = base_output_shapes + [int(256 * m), int(256 * m), int(128 * m)]
        super().__init__(backbone, [4, 5], out_channels)
        assert len(out_channels) == 5

        self.conv0 = DepthwiseSeparableConv2d(out_channels[1], out_channels[2], kernel_size=3, stride=2, padding=1)
        self.conv1 = DepthwiseSeparableConv2d(out_channels[2], out_channels[3], kernel_size=3)
        self.conv2 = DepthwiseSeparableConv2d(out_channels[3], out_channels[4], kernel_size=3)
