...
class FeaturePyramidNetworkLite4(Head):
    def __init__(self, backbone, out_channels=256):
        super().__init__(backbone, [5, 4], [out_channels] * 4)
        base_output_shapes = Head.get_base_output_shapes(backbone, [5, 4])
        ...
        self.conv2_0 = Conv2dAct(base_output_shapes[0], base_output_shapes[0], kernel_size=3, padding=1, stride=2, groups=base_output_shapes[0])
        self.conv2_1 = torch.nn.Conv2d(base_output_shapes[0], out_channels, kernel_size=1)  # P_6

        self.conv3_0 = Conv2dAct(out_channels, out_channels, kernel_size=3, padding=1, stride=2, groups=out_channels)
        self.conv3_1 = torch.nn.Conv2d(out_channels, out_channels, kernel_size=1)  # P_7
    ...
    def forward(self, input):
        ...
        x = self.conv2_0(base_features[0])  # C5
        p6 = self.conv2_1(x)

        x = self.activation(p6)
        p7 = self.conv3_1(self.conv3_0(x))
